...
import io.github.japskiddin.utils.decrypt
import io.github.japskiddin.utils.deleteDir
import io.github.japskiddin.utils.encrypt
import java.io.File
...
class CipherWorker {
  ...
  fun checkArguments(args: Array<String>) {
    ...
    doubleOptsList.forEach { opt ->
      when (opt) {
        ...
        "help" -> showHelp()
        "decrypt" -> doCipher(Cipher.DECRYPT_MODE, src, dst, key)
        "encrypt" -> doCipher(Cipher.ENCRYPT_MODE, src, dst, key)
      }
    }
  }
   ...
   */
  private fun doCipher(type: Int, src: String?, dst: String?, key: String?) {
      ...
      if (srcFile.isDirectory) {
        ...
        try {
          if (type == Cipher.ENCRYPT_MODE) {
            encrypt(key, srcFile, dstFile)
          } else {
            decrypt(key, srcFile, dstFile)
          }
